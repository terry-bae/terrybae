...
void load_pdata(struct member* info); //회원정보 파일 로드 함수 선언
void save_pdata(struct member* info); //회원정보 파일 세이브 함수 선언
...
void load_pdata(struct member* info)       /////회원정보 데이터 파일 로
{
    int i;
    FILE* fp = fopen("member_info.txt", "rt");
    if (fp == NULL)
...
void signup(struct member* info) //회원가입 signup 함수 선언
{
    int i, j, k;
...
void save_pdata(struct member* info)  ////회원정보 파일 세이브
{
    ...
    if (flag == 1) {
        ...
        if (temp != 0) { //현재 저장된 회원이 있다면
            FILE* fp3 = fopen("member_info.txt", "r+");  //파일을 읽기/쓰기 모드로 열기
        ...
        FILE* fp = fopen("member_info.txt", "wt");
